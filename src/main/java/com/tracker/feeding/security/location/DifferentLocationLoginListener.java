...
public class DifferentLocationLoginListener implements ApplicationListener<DifferentLocationLoginHandler> {
    ...
    public void onApplicationEvent(final DifferentLocationLoginListener event) {
        ...
        final String subject = "Login attempt from an unknown location.";
        final String message = messages.getMessage("message.differentLocation",  new Object[] { new Date().toString(), event.getToken().getUserLocation().getCountry(), event.getIp(), enableLocUri, changePassUri }, event.getLocale());
